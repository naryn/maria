...
var (
	...
	hmm     = app.StringOpt("hmm", "/usr/local/share/pocketsphinx/model/zh-cn/zh/tdt_sc_8k/", "Sets directory containing acoustic model files.")
	dict    = app.StringOpt("dict", "/usr/local/share/pocketsphinx/model/zh-cn/9631.dic", "Sets main pronunciation dictionary (lexicon) input file..")
	lm      = app.StringOpt("lm", "/usr/local/share/pocketsphinx/model/zh-cn/9631.lm", "Sets word trigram language model input file.")
...
func init() {
	fmt.Println("init pocketSphinx")
	stt := stt_pocketSphinx{}
	Regist("pocketSphinx", stt)
}

type stt_pocketSphinx struct {
}

func (this stt_pocketSphinx) Start() {
	fmt.Println("pocketSphinx start")
	defer closer.Close()
	closer.Bind(func() {
		log.Println("Bye!")
	})
...
func (l *Listener) report() {
	...
	if len(hyp) > 0 {
		log.Printf("识别到: %s", hyp)
		conversation.Msg <-hyp
		return
	}
